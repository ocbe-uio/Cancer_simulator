...
eval_function <- function(mean_sd){
  ...
  mean_diff <-abs((mean - mean(sample))/mean)
  sd_diff <- abs((sd - sd(sample))/sd)
  mean_diff + sd_diff
}
...
SimSumFG<-function(CellTables,cdDF,mainDF,SimTime,AA,CellDeath,RCellNum,
                   LatticeTable,CellSenescence,MCells,G1Division,FinalAveSpe,
                   DividingCells,NotMotileCells,RanNum){
  simsumDF<-data.frame()
  ...
  simsumDF[1:length(totalSteps),8]<-TTIRZ
  simsumDF[1:length(totalSteps),9]<- UIVALUES
  simsumDF[1:length(totalSteps),10]<- c(rep(NA,(length(totalSteps)-1)),length(visits))
  simsumDF[1:length(totalSteps),11]<- Metastasis
  simsumDF[1:length(totalSteps),12]<- c(rep(NA,(length(totalSteps)-1)),FinalAveSpe)
  simsumDF[1:length(totalSteps),13]<- c(rep(NA,(length(totalSteps)-1)),length(G1Division))
  simsumDF[1:length(totalSteps),14]<- c(rep(NA,(length(totalSteps)-1)),NotMotileCells)
  simsumDF[1:length(totalSteps),15]<- c(rep(NA,(length(totalSteps)-1)),DividingCells)

  colnames(simsumDF)<-c("Steps","Number of cells","Number of cells in RedZone","RedZone cells ratio","MaxCellID",
                        "Cumulative Dead Cells","Cumulative Senescent cells","Cumulative time in RedZone",
                        "Uniformity Index","Total Number of RedZone Visitors","Cumulative Metastatic cells ",
                        "Average speed of all cells","Cumulative G1B cells","L5S Immotile Cells","L5S Dividing Cells")
...
UIplotFG<-function(TS,LatticeTable,RanNum){
  lat1<-LatticeTable[TS,]
  smallBoxes<- c(0:49)
  ...
  jpeg(paste0("UniformityPlot-","R",RanNum,".jpeg"),width = 6, height = 6, units = 'in', res = 500)
  barplot(allLATs, main=paste0("UniformityPlot-","R",RanNum), xlab="Square Quadrat",ylab="Number of cells per Square Quadrat",sub=(paste0("Uniformity Index =",UI,"%"))  )
  dev.off()
...
PlottingCellsADLastStepFG<- function(cdDF,mainDF,SimTime,RanNum,VesselsXY,fFullTable,
                                     col1, col2, col3, col4, BloVesN,
                                     Rrows){
  ...
  for (i in 1:totalnumCell){
    fromfull<-as.numeric(fFullTable[,1])
    if (fromfull[i]==1){
      color[i]=col1
    }
    if (fromfull[i]==2){
  ...
  jpeg(paste0("Track plot-",s,"R",RanNum,".jpeg"),width = 6, height = 6, units = 'in', res = 500)
  plot (0,0, type = "n",xlim=c(0,4000),ylim=c(0,4000),xlab="X",ylab="Y",main=paste0(s," Steps   ",length(X), "cells"))
...
PlottingCellsADFG<- function(cdDF,mainDF,SimTime,RanNum,VesselsXY,fFullTable,
                             col1, col2, col3, col4, BloVesN,
                             Rrows){
...
SimulationScriptM2<- function(TimeInterval,DT,DTinR,VesselsXY,LatDF,PeriodForSenescence,
                              PeriodForDeath,LeavingRzone,IntravasationProb,FullTable,
                              mainDF,cdDF,phylogenicDF,CellTables,speedTable,MetastatisTable,
                              SimTime,exportSP=FALSE,exportRdata=FALSE,
                              col1, col2, col3, col4, BloVesN, Rrows){
  ...
  for (step in (LenTar+1):SimTime){
    ...
    if (length(NewCellsID) >1){
      ...
      for(g in gone){
        ...
        if (Dens[[1]]==-2  || (Dens[[1]]==-1 && Dens[[2]]  %in% black) ){
          ...
          RanM<-runif(1,0,1)
          MRatio<-c()
          MRatio<-as.numeric(FullTable[3,MAX+ options[2]])

          if (BSecondDC %in% black  && RanM>MRatio){
    ...
    if (length(ifold) >0){
      for (o in ifold){
        ...
        ############
        MRatio<-c()
        MRatio<-as.numeric(FullTable[3,o])
        RanM<-runif(1,0,1)                       ##########The cell will stay in place
        if (BB %in% black  && RanM>MRatio){
          BB<-B
          NX<-CellTables[[o]][Ma-1,3]
          NY<-CellTables[[o]][Ma-1,4]
        ...
        if (BB %in% black  && RanM<=MRatio){
          ...
          CellTables[[o]][Ma,14]=CellTables[[o]][Ma-1,14]
    ...
    for (i in 1:length(CellTables)){
      ...
      if (lenNumStep>=460){CelDet[i]=1}else{CelDet[i]=NA}
    }
  ...
  write.csv(CellNumberPerDivision,file=paste0("CellNumberPerDivision","R",RanNum,".csv"))
  ...
  fFullTable<-FullTable[1:4,1:totalnumCell]
  ########################## computing the average speed of each cell
  for (i in 1:totalnumCell){
    SPEED<-CellTables[[i]][,14]
    SPEED<-SPEED[!is.na(SPEED)]
    for(s in SPEED){
    ...
    fFullTable[5,i]=round(mean(SPEED),digits=2)
  }
  ...
  for(d in MaxCell){
    targetCell<-CellTables[[d]][,7]
    if (length(targetCell[!is.na(targetCell)])>0 && (1 %in% targetCell[!is.na(targetCell)] || "1" %in% targetCell[!is.na(targetCell)])){
      fFullTable[6,d]=1
    }else{
      fFullTable[6,d]=NA
    }
  ...
  for(d in MaxCell){
    targetCell<-CellTables[[d]][,7]
    if (length(targetCell[!is.na(targetCell)])>0 && (1 %in% targetCell[!is.na(targetCell)] || "1" %in% targetCell[!is.na(targetCell)])){
      fFullTable[7,d]=sum(CellTables[[d]][,7],na.rm=T)
    }else{
      fFullTable[7,d]=NA
    }
    fFullTable[8,d]=NA
  }
  MetastaticCells<-MetastatisTable[SimTime,!is.na(MetastatisTable[SimTime,])]
  for (n in MetastaticCells){
    fFullTable[8,n]="1"
  }

  fFullTable=t(fFullTable[1:8,])
  colnames(fFullTable)<-c("GroupID","Golgi_State","IntravasationProb","PER","Average_Speed","Visited_Rzone","Hours_in_Rzone","Metastatic_Celle")


  simsumDF<-SimSumFG(CellTables,cdDF,mainDF,SimTime,AA,CellDeath,RCellNum,LatticeTable,CellSenescence,MCells,G1Division,FinalAveSpe,
                     DividingCells,NotMotileCells,RanNum)

  write.csv(phylogenicDF,file=paste0("phylogenicDF","R",RanNum,".csv"))
  write.csv(fFullTable,file=paste0("fFullTable","R",RanNum,".csv"))

  jpeg(paste0("G1B Cells"," Number of G1B cells=",length(G1Division),"R",RanNum,".jpeg"),width = 6, height = 6, units = 'in', res = 500)
  G1B.Cells=c(1:length(G1Division))
  CellID=G1Division
  LensimsumDF<-length(simsumDF[,1])
  plot(G1B.Cells,CellID,type="p",ylim=c(1,simsumDF[LensimsumDF,5]),main=paste0("G1Bcells=",length(G1Division),", Tumor cells=",simsumDF[LensimsumDF,5]),cex.main=1)
  ...
  PlottingCellsADLastStepFG(cdDF,mainDF,SimTime,RanNum,VesselsXY,fFullTable,
                            col1, col2, col3, col4, BloVesN, Rrows)
  jpeg(paste0("CellNumberPerDivision-",s,"R",RanNum,".jpeg"),width = 6, height = 6, units = 'in', res = 500)
  plot(divisions,CellNumberPerDivision,type="o",ylab="Number of cells undergone cell division",ylim=c(0,30000),
       xlab="Cell division number",pch=16,main=paste0("Cell Division Curve"))
  dev.off()
  UIplotFG(TS,LatticeTable,RanNum)

  if (exportSP== TRUE) {
    PlottingCellsADFG(cdDF,mainDF,SimTime,RanNum,VesselsXY,fFullTable, col1, col2, col3, col4, BloVesN, Rrows)

  }
  if (exportRdata== TRUE) {
    ScdDF<-cdDF[,1:totalnumCell]
    SmainDF<-mainDF[,1:totalnumCell]
    save(ScdDF, file=paste0("cdDF-","R",RanNum,".RData"))
    save(SmainDF, file=paste0("mainDF-","R",RanNum,".RData"))
    save(fFullTable, file = paste0(RanNum,"fFullTable.RData"))
...
#
# Run Model #2
RunCellmigSimulation_M2 <- function(
  ...
  PeriodForSenescence=2,
  PeriodForDeath=3,
  LeavingRzone=0.5,

  IntravasationProb=0.01,
  IntraVas_inc_By_GF =33,
  ...
  exportRdata=FALSE)
{
  ...
  ## Control groups
  ###First Group: Frequency, Speed, Persistence, color
  Frequency1C<- 32.6

  minSpe1C<- 1.944             # um/h
  maxSpe1C<- 80.703            # um/h
  desired_meanSpe1C<- 24.033   # um/h
  desired_sdSpe1C<- 2.854      # um/h
  PER1C<- 0.604                # The persistence value
  col1<- "yellow"

  ###Second Group: Frequency, Speed and Persistence
  Frequency2C<- 21.1
  minSpe2C<- 2.154             # um/h
  maxSpe2C<- 86.964            # um/h
  desired_meanSpe2C<- 25.241   # um/h
  desired_sdSpe2C<- 2.31       # um/h
  PER2C<- 0.725                # The persistence value
  col2<- "green"

  ###Third Group: Frequency, Speed and Persistence
  Frequency3C<- 13.8
  minSpe3C<- 2.812               # um/h
  maxSpe3C<- 106.118             # um/h
  desired_meanSpe3C<- 31.43      # um/h
  desired_sdSpe3C<- 2.865        # um/h
  PER3C<- 0.601                  # The persistence value
  col3<- "blue"

  ###Fouth Group: Frequency, Speed and Persistence
  Frequency4C<- 32.5
  minSpe4C<- 2.933              # um/h
  maxSpe4C<- 107.514            # um/h
  desired_meanSpe4C<- 33.774    # um/h
  desired_sdSpe4C<- 4.479       # um/h
  PER4C<- 0.756                 # The persistence value
  col4<- "gray"

  ############################## Fragmented Golgi groups
  ###First Group: Frequency, Speed, Persistence, color
  Frequency1G<- 42.4
  minSpe1G <- 1.944             # um/h
  maxSpe1G <- 80.703            # um/h
  desired_meanSpe1G <- 21.433   # um/h
  desired_sdSpe1G <- 3.368      # um/h
  PER1G <- 0.607                # The persistence value
  col1<- "yellow"

  ###Second Group: Frequency, Speed and Persistence
  Frequency2G<- 38.1
  minSpe2G <- 2.154             # um/h
  maxSpe2G <- 86.964            # um/h
  desired_meanSpe2G <- 23.841   # um/h
  desired_sdSpe2G <- 2.744      # um/h
  PER2G <- 0.737                # The persistence value
  col2<- "green"

  ###Third Group: Frequency, Speed and Persistence
  Frequency3G<- 2.9
  minSpe3G <- 2.812               # um/h
  maxSpe3G <- 106.118             # um/h
  desired_meanSpe3G <- 30.617     # um/h
  desired_sdSpe3G <- 2.376        # um/h
  PER3G <- 0.63                 # The persistence value
  col3<- "blue"

  ###Fouth Group: Frequency, Speed and Persistence
  Frequency4G<- 16.6
  minSpe4G <- 2.933              # um/h
  maxSpe4G <- 107.514            # um/h
  desired_meanSpe4G <- 32.71     # um/h
  desired_sdSpe4G <- 4.123       # um/h
  PER4G <- 0.78                  # The persistence value
  col4<- "gray"


  ### Estimating the total number of generated cells, it will be used for making the tables
  ...
  ### Making individual tables in a list    where each table has a ranged seepd profile
  ########### Function to generate speeds based on mean
  eval_function <- function(mean_sd){
    mean <- mean_sd[1]
    sd <- mean_sd[2]
    sample <- rtruncnorm(n = DT, a = minSpe, b = maxSpe, mean = mean, sd = sd)
  ...
  ###### Generating a "speedTable" for each group for Fragmented Golgi
  speedTable1G<-data.frame()
  minSpe=minSpe1G
  maxSpe=maxSpe1G
  for (i in 1:1000){                  ########## generating 1000 speed profiles
    o <- optim(c(desired_meanSpe1G, desired_sdSpe1G), eval_function)
    Speed <- rtruncnorm(n = DT, a = minSpe, b = maxSpe, mean = o$par[1], sd = o$par[2])
    SpeedT<-c()
    for (d in 1:PeriodForDeath){
      SpeedT<-c(SpeedT,Speed)
    }
    LenSpeedT<-length(SpeedT)
    speedTable1G[1:LenSpeedT,i]= round(SpeedT,digits=2)
  }

  speedTable2G<-data.frame()
  minSpe=minSpe2G
  maxSpe=maxSpe2G
  for (i in 1:1000){                  ########## generating 1000 speed profiles
    o <- optim(c(desired_meanSpe2G, desired_sdSpe2G), eval_function)
    Speed <- rtruncnorm(n = DT, a = minSpe, b = maxSpe, mean = o$par[1], sd = o$par[2])
    SpeedT<-c()
    for (d in 1:PeriodForDeath){
      SpeedT<-c(SpeedT,Speed)
    ...
    LenSpeedT<-length(SpeedT)
    speedTable2G[1:LenSpeedT,i]= round(SpeedT,digits=2)
  }

  speedTable3G<-data.frame()
  minSpe=minSpe3G
  maxSpe=maxSpe3G
  for (i in 1:1000){                  ########## generating 1000 speed profiles
    o <- optim(c(desired_meanSpe3G, desired_sdSpe3G), eval_function)
    Speed <- rtruncnorm(n = DT, a = minSpe, b = maxSpe, mean = o$par[1], sd = o$par[2])
    SpeedT<-c()
    for (d in 1:PeriodForDeath){
      SpeedT<-c(SpeedT,Speed)
    ...
    LenSpeedT<-length(SpeedT)
    speedTable3G[1:LenSpeedT,i]= round(SpeedT,digits=2)
  }

  speedTable4G<-data.frame()
  minSpe=minSpe4G
  maxSpe=maxSpe4G
  for (i in 1:1000){                  ########## generating 1000 speed profiles
    o <- optim(c(desired_meanSpe4G, desired_sdSpe4G), eval_function)
    Speed <- rtruncnorm(n = DT, a = minSpe, b = maxSpe, mean = o$par[1], sd = o$par[2])
    SpeedT<-c()
    for (d in 1:PeriodForDeath){
      SpeedT<-c(SpeedT,Speed)
    ...
    LenSpeedT<-length(SpeedT)
    speedTable4G[1:LenSpeedT,i]= round(SpeedT,digits=2)
  }

  # Second part
  speedTable1C<-data.frame()
  minSpe=minSpe1C
  maxSpe=maxSpe1C
  for (i in 1:1000){                  ########## generating 1000 speed profiles
    o <- optim(c(desired_meanSpe1C, desired_sdSpe1C), eval_function)
    Speed <- rtruncnorm(n = DT, a = minSpe, b = maxSpe, mean = o$par[1], sd = o$par[2])
    SpeedT<-c()
    for (d in 1:PeriodForDeath){
      SpeedT<-c(SpeedT,Speed)
    ...
    LenSpeedT<-length(SpeedT)
    speedTable1C[1:LenSpeedT,i]= round(SpeedT,digits=2)
  }

  speedTable2C<-data.frame()
  minSpe=minSpe2C
  maxSpe=maxSpe2C
  for (i in 1:1000){                  ########## generating 1000 speed profiles
    o <- optim(c(desired_meanSpe2C, desired_sdSpe2C), eval_function)
    Speed <- rtruncnorm(n = DT, a = minSpe, b = maxSpe, mean = o$par[1], sd = o$par[2])
    SpeedT<-c()
    for (d in 1:PeriodForDeath){
      SpeedT<-c(SpeedT,Speed)
    ...
    LenSpeedT<-length(SpeedT)
    speedTable2C[1:LenSpeedT,i]= round(SpeedT,digits=2)
  }

  speedTable3C<-data.frame()
  minSpe=minSpe3C
  maxSpe=maxSpe3C
  for (i in 1:1000){                  ########## generating 1000 speed profiles
    o <- optim(c(desired_meanSpe3C, desired_sdSpe3C), eval_function)
    Speed <- rtruncnorm(n = DT, a = minSpe, b = maxSpe, mean = o$par[1], sd = o$par[2])
    SpeedT<-c()
    for (d in 1:PeriodForDeath){
      SpeedT<-c(SpeedT,Speed)
    ...
    LenSpeedT<-length(SpeedT)
    speedTable3C[1:LenSpeedT,i]= round(SpeedT,digits=2)
  }

  speedTable4C<-data.frame()
  minSpe=minSpe4C
  maxSpe=maxSpe4C
  for (i in 1:1000){                  ########## generating 1000 speed profiles
    o <- optim(c(desired_meanSpe4C, desired_sdSpe4C), eval_function)
    Speed <- rtruncnorm(n = DT, a = minSpe, b = maxSpe, mean = o$par[1], sd = o$par[2])
    SpeedT<-c()
    for (d in 1:PeriodForDeath){
      SpeedT<-c(SpeedT,Speed)
    ...
    LenSpeedT<-length(SpeedT)
    speedTable4C[1:LenSpeedT,i]= round(SpeedT,digits=2)
  }
  ...
  ##################################################################
  ##################################################################
  disIntMed<-c(rep(1,Frequency1G),rep(2,Frequency2G),rep(3,Frequency3G),rep(4,Frequency4G))
  Intact<- 100 -Golgi_Fra_Frequency
  Golgi_Fra_Freq<-c(rep("I",Intact),rep("F",Golgi_Fra_Frequency))

  CellmaxPeriod<-DT *PeriodForDeath
  FullTable<-data.frame()
  for (i in 1:EstCellN){
    FullTable[1,i]<- sample(disIntMed,1)
    FullTable[2,i]<- sample(Golgi_Fra_Freq,1)
    if (FullTable[2,i]=="F"){
      RanDi= round(runif(1,minRange_IntraVas_inc_By_GF,maxRange_IntraVas_inc_By_GF),digits=2)
      FullTable[3,i]<- RanDi * IntravasationProb * IntraVas_inc_By_GF
    }else{
      FullTable[3,i]<- IntravasationProb
    }
    if (FullTable[1,i]==1){
      FullTable[4,i]<-PER1G
    }
    else if (FullTable[1,i]==2){
      FullTable[4,i]<-PER2G
    }
    else if (FullTable[1,i]==3){
      FullTable[4,i]<-PER3G
    }
    else if (FullTable[1,i]==4){
      FullTable[4,i]<-PER4G
    }
    RanSpeed= round(runif(1,1,100))
    if (FullTable[1,i]==1){
      FullTable[5:(CellmaxPeriod+4),i]<- speedTable1G[1:CellmaxPeriod,RanSpeed]
    }
    else if (FullTable[1,i]==2){
      FullTable[5:(CellmaxPeriod+4),i]<- speedTable2G[1:CellmaxPeriod,RanSpeed]
    }
    else if (FullTable[1,i]==3){
      FullTable[5:(CellmaxPeriod+4),i]<- speedTable3G[1:CellmaxPeriod,RanSpeed]
    }
    else if (FullTable[1,i]==4){
      FullTable[5:(CellmaxPeriod+4),i]<- speedTable4G[1:CellmaxPeriod,RanSpeed]
    }
  }
  ...
  for (n in 1:EstCellN){
    CellTables [[n]]<- data.frame(matrix(NA, nrow = CellmaxPeriod, ncol = 17))

    CellTables [[n]][2:LenSpeedT,13]= as.numeric(FullTable[(5:(LenSpeedT+3)),n])
    CellTables [[n]][2:LenSpeedT,8]=c(1:(LenSpeedT-1))
    colnames(CellTables [[n]])<-
      c("ID","Steps","X","Y","B","Density","Red","CellStep","NotRed",
        "Sum","RanDR","RanRed","Pre.Distance (um)",
        "Actual Distance (um)","Movement options","CD")
  }
  ############### Preparing the first table
  ...
  y0 <- SimulationScriptM2(
    ...
    mainDF = mainDF,
    cdDF = cdDF,
    phylogenicDF = phylogenicDF,
    CellTables = CellTables,
    speedTable = speedTable,
    MetastatisTable = MetastatisTable,
    SimTime = SimTime,
    exportSP =exportSP,
